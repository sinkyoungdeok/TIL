...
- 2번째: jvm warm up
  ...
  - warm up에 부하가 많이 가는 쿼리를 넣으려면 어떻게 해야할지.
    - 지금까지 생각나는건 warm up 쿼리에 timeout 값을 100ms정돌르 주고서 진행하는 것이다.
    - 근데, 이것도 부하를 많이 줄 수도 있어서 더 좋은 방법을 찾아야함. 
...
- 만약 항상 같은 결과를 내야하는 상황에서는 낮은 latency로 조절해놓는게 위험할 수 있다. 이럴 땐 서버 timeout 설정과 동일하게 세팅하는것이 좋다.
