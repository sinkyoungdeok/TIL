...
- [1. 설치 명령어](#1-설치-명령어)
  - [Docker 설치 명령어 (Mac OS)](#docker-설치-명령어-mac-os)
- [2. 기본 개념](#2-기본-개념)
  - [이미지 (Image)](#이미지-image)
  - [컨테이너 (Container)](#컨테이너-container)
  - [이미지와 컨테이너의 관계](#이미지와-컨테이너의-관계)
  - [도커 이미지 이름 구성](#도커-이미지-이름-구성)
  - [도커 이미지 저장소](#도커-이미지-저장소)
  - [도커 구성 요소](#도커-구성-요소)
  - [도커 컨테이너 라이프사이클](#도커-컨테이너-라이프사이클)
  - [엔트리 포인트](#엔트리-포인트)
  - [커맨드](#커맨드)
  - [도커 네트워크 구조](#도커-네트워크-구조)
  - [도커 네트워크 드라이버](#도커-네트워크-드라이버)
  - [도커 로그 드라이버](#도커-로그-드라이버)
  - [도커 이미지 구조](#도커-이미지-구조)
  - [도커 컴포즈 개념](#도커-컴포즈-개념)
  - [도커 컴포즈 - 프로젝트](#도커-컴포즈---프로젝트)
  - [도커 컴포즈 - 서비스](#도커-컴포즈---서비스)
  - [도커 컴포즈 - 컨테이너](#도커-컴포즈---컨테이너)
  - [도커 컴포즈 - 주요 사용 목적](#도커-컴포즈---주요-사용-목적)
- [3. 기본 명령어](#3-기본-명령어)
  - [컨테이너 생성](#컨테이너-생성)
  - [컨테이너 생성 및 시작](#컨테이너-생성-및-시작)
  - [컨테이너 시작](#컨테이너-시작)
  - [버전 확인](#버전-확인)
  - [실행 중인 컨테이너 확인](#실행-중인-컨테이너-확인)
  - [컨테이너 상세 정보 확인](#컨테이너-상세-정보-확인)
  - [컨테이너 일시 중지](#컨테이너-일시-중지)
  - [컨테이너 재개](#컨테이너-재개)
  - [컨테이너 종료 (SIGTERM)](#컨테이너-종료-sigterm)
  - [컨테이너 강제 종료 (SIGKILL)](#컨테이너-강제-종료-sigkill)
  - [모든 컨테이너 종료](#모든-컨테이너-종료)
  - [컨테이너 삭제](#컨테이너-삭제)
  - [컨테이너 강제 종료 후 삭제 (SIGKILL)](#컨테이너-강제-종료-후-삭제-sigkill)
  - [컨테이너 종료 후 자동 삭제](#컨테이너-종료-후-자동-삭제)
  - [중지된 모든 컨테이너 삭제](#중지된-모든-컨테이너-삭제)
  - [이미지 모두 삭제](#이미지-모두-삭제)
  - [엔트리포인트와 커맨드](#엔트리포인트와-커맨드)
  - [환경 변수](#환경-변수)
  - [실행중인 컨테이너에 명령어를 실행](#실행중인-컨테이너에-명령어를-실행)
- [4. 포트 관련 명령어](#4-포트-관련-명령어)
  - [컨테이너 포트 노출](#컨테이너-포트-노출)
  - [expose](#expose)
- [5. 네트워크 관련 명령어](#5-네트워크-관련-명령어)
  - [network none](#network-none)
  - [network host](#network-host)
  - [bridge network 생성](#bridge-network-생성)
...
- [7. 로그 관련 명령어](#7-로그-관련-명령어)
  - [로그 확인하기](#로그-확인하기)
  - [호스트 운영체제의 로그 저장 경로](#호스트-운영체제의-로그-저장-경로)
  - [로그 용량 제한](#로그-용량-제한)
- [8. 이미지 빌드 명령어](#8-이미지-빌드-명령어)
  - [Dockerfile 없이 이미지 생성](#dockerfile-없이-이미지-생성)
...
- [10. 이미지 압축파일 저장 명령어](#10-이미지-압축파일-저장-명령어)
  - [이미지 tar 압축파일로 저장](#이미지-tar-압축파일로-저장)
  - [이미지 압축에서 불러오기](#이미지-압축에서-불러오기)
- [11. 도커 허브 명령어](#11-도커-허브-명령어)
  - [허브 로그인](#허브-로그인)
  - [도커 허브 repository에 push](#도커-허브-repository에-push)
- [12. AWS ECR 저장소 명령어](#12-aws-ecr-저장소-명령어)
  - [AWS ECR push](#aws-ecr-push)
- [13. 이미지 경량화 전략](#13-이미지-경량화-전략)
  - [꼭 필요한 패키지 및 파일만 추가](#꼭-필요한-패키지-및-파일만-추가)
...
- [14. 도커 데몬 디버깅](#14-도커-데몬-디버깅)
  ...
  - [도커 저장된 공간 확보](#도커-저장된-공간-확보)
  - [컨테이너별 cpu 메모리 네트워크io사용률 등 보기](#컨테이너별-cpu-메모리-네트워크io사용률-등-보기)
- [15. 도커 컴포즈 명령어](#15-도커-컴포즈-명령어)
  - [version](#version)
  - [도커 스왐 (Docker Swarm)](#도커-스왐-docker-swarm)
  - [services](#services)
  - [networks](#networks)
  - [volumes](#volumes)
  - [프로젝트 목록 확인](#프로젝트-목록-확인)
  - [도커 컴포즈 파일 예시](#도커-컴포즈-파일-예시)
  - [도커 컴포즈 실행](#도커-컴포즈-실행)
  - [프로젝트 내 컨테이너 및 네트워크 종료 및 제거](#프로젝트-내-컨테이너-및-네트워크-종료-및-제거)
  - [프로젝트 내 컨테이너, 네트워크 및 볼륨 종료 및 제거](#프로젝트-내-컨테이너-네트워크-및-볼륨-종료-및-제거)
  - [docker-compose 서비스 확장](#docker-compose-서비스-확장)
  - [프로젝트 내 서비스 로그 확인](#프로젝트-내-서비스-로그-확인)
  - [프로젝트 내 컨테이너 이벤트 확인](#프로젝트-내-컨테이너-이벤트-확인)
  - [프로젝트 내 이미지 목록](#프로젝트-내-이미지-목록)
  - [프로젝트 내 컨테이너 목록](#프로젝트-내-컨테이너-목록)
  - [프로젝트 내 실행중인 프로세스 목록](#프로젝트-내-실행중인-프로세스-목록)
  - [도커 컴포즈 파일 내 옵션들](#도커-컴포즈-파일-내-옵션들)
- [16. 도커 컴포즈 활용 예시](#16-도커-컴포즈-활용-예시)
  - [1. Grafana 구성하기](#1-grafana-구성하기)
  - [2. Grafana + Mysql 구성하기](#2-grafana--mysql-구성하기)
- [17. jib을 이용한 docker 이미지 빌드 푸시](#17-jib을-이용한-docker-이미지-빌드-푸시)
  - [빌드란 무엇인가?](#빌드란-무엇인가)
  - [빌드 방식](#빌드-방식)
  - [Gradle이란?](#gradle이란)
  - [Gradle 프로젝트의 구조](#gradle-프로젝트의-구조)
  - [Jib이란](#jib이란)
  - [Jib과 기존 Dockerfile 기반 빌드 비교](#jib과-기존-dockerfile-기반-빌드-비교)
  - [Jib 설정](#jib-설정)
  - [Gradle 설치](#gradle-설치)
  - [Gradle Project 생성 및 빌드](#gradle-project-생성-및-빌드)
  - [Docker 빌드 및 Push](#docker-빌드-및-push)
  - [실습 파일](#실습-파일)
- [18. jenkins 소개와 설치](#18-jenkins-소개와-설치)
  - [jenkins 소개](#jenkins-소개)
  - [jenkins 특징](#jenkins-특징)
  - [jenkins 플러그인 소개](#jenkins-플러그인-소개)
  - [jenkins 설치 명령어](#jenkins-설치-명령어)
  - [jenkins 플러그인 설치](#jenkins-플러그인-설치)
  - [SSH Key 생성 및 Github 적용](#ssh-key-생성-및-github-적용)
  - [Jenkins SSH key 추가](#jenkins-ssh-key-추가)
  - [AWS key 추가](#aws-key-추가)
  - [deploy 서버의 pem키를 jenkins의 deploy ssh key로 추가](#deploy-서버의-pem키를-jenkins의-deploy-ssh-key로-추가)
- [19. jenkins를 활용한 docker 빌드](#19-jenkins를-활용한-docker-빌드)
  - [jenkins CI/CD job 수행 및 Docker 빌드/배포 과정](#jenkins-cicd-job-수행-및-docker-빌드배포-과정)
  - [파이프라인 구성](#파이프라인-구성)
  - [Jenkinsfile 설명](#jenkinsfile-설명)
  - [배포할 서버에 aws 인증](#배포할-서버에-aws-인증)
  - [배포](#배포)
  - [jenkins 진행 repository](#jenkins-진행-repository)
- [20. nexus를 활용한 docker 빌드](#20-nexus를-활용한-docker-빌드)
  - [nexus 소개](#nexus-소개)
  - [nexus 장점](#nexus-장점)
  - [Nexus 와 Jenkins CI 연동](#nexus-와-jenkins-ci-연동)
  - [Nexus Repository](#nexus-repository)
  - [Nexus Repository Type](#nexus-repository-type)
  - [Nexus Repository 구성 방식](#nexus-repository-구성-방식)
  - [Nexus 설치 명령어](#nexus-설치-명령어)
  - [Nexus Admin Password 확인 명령어](#nexus-admin-password-확인-명령어)
  - [Blob Store를 S3에 연동](#blob-store를-s3에-연동)
  - [Proxy 타입의 Repository 생성](#proxy-타입의-repository-생성)
  - [Nexus Artifact를 활용한 Docker 빌드](#nexus-artifact를-활용한-docker-빌드)
  - [실습 파일](#실습-파일-1)
- [21. sonarqube를 활용한 docker 빌드](#21-sonarqube를-활용한-docker-빌드)
  - [정적 분석 vs 동적 분석](#정적-분석-vs-동적-분석)
  - [sonarqube 소개](#sonarqube-소개)
  - [sonarqube 구성 방식](#sonarqube-구성-방식)
  - [sonarqube 기능](#sonarqube-기능)
  - [Sonarqube & Jenkins CI 연동](#sonarqube--jenkins-ci-연동)
  - [SonarQube 설치 명령어](#sonarqube-설치-명령어)
  - [SonarQube 유저 생성](#sonarqube-유저-생성)
  - [Jacoco란](#jacoco란)
  - [SonarQube & Jacoco 관련 명령어](#sonarqube--jacoco-관련-명령어)
  - [Gradle에서 SonarQube 및 Jacoco 플러그인 설정](#gradle에서-sonarqube-및-jacoco-플러그인-설정)
  - [Jacoco 코드 커버리지 측정 및 리포트 결과](#jacoco-코드-커버리지-측정-및-리포트-결과)
  - [SonarQube에서 정적코드 품질분석 결과확인](#sonarqube에서-정적코드-품질분석-결과확인)
  - [실습 파일](#실습-파일-2)
- [22. docker 컨테이너 보안을 위한 clair 활용](#22-docker-컨테이너-보안을-위한-clair-활용)
  - [Docker 컨테이너 보안](#docker-컨테이너-보안)
  - [Clair 소개](#clair-소개)
  - [Clair 아키텍처](#clair-아키텍처)
  - [Clair 작동 방식](#clair-작동-방식)
  - [Clair를 통한 컨테이너 이미지 분석 방식](#clair를-통한-컨테이너-이미지-분석-방식)
  - [Clairctl를 통한 보안 취약점 스캔](#clairctl를-통한-보안-취약점-스캔)
  - [Clair & Jenkins CI 연동](#clair--jenkins-ci-연동)
  - [Clair 설치](#clair-설치)
  - [Clair 관련 명령어](#clair-관련-명령어)
  - [Clair-scanner 기본 보안 설정을 위한 옵션](#clair-scanner-기본-보안-설정을-위한-옵션)
  - [Jenkins에서 많이 사용하는 Docker 이미지 스캔 및 결과 확인](#jenkins에서-많이-사용하는-docker-이미지-스캔-및-결과-확인)
  - [Gradle-Jib 예제 Docker 이미지 빌드 스캔 및 결과 확인](#gradle-jib-예제-docker-이미지-빌드-스캔-및-결과-확인)
- [23. docker 컨테이너 빌드 및 푸시 방법 nexus vs aws ecr](#23-docker-컨테이너-빌드-및-푸시-방법-nexus-vs-aws-ecr)
  - [Docker 컨테이너 빌드 후 Nexus에 이미지 Push 및 Clair 보안 스캔 수행](#docker-컨테이너-빌드-후-nexus에-이미지-push-및-clair-보안-스캔-수행)
  - [Docker 컨테이너 빌드 후 AWS ECR에 이미지 Push 및 ECR 보안 스캔 수행](#docker-컨테이너-빌드-후-aws-ecr에-이미지-push-및-ecr-보안-스캔-수행)
  - [Nexus Docker Registry 설치 및 설정](#nexus-docker-registry-설치-및-설정)
  - [Nexus Docker 빌드 및 Push&Pull 수행](#nexus-docker-빌드-및-pushpull-수행)
  - [Nexus Docker 명령어](#nexus-docker-명령어)
  - [AWS ECR Repository 보안 취약점 스캔 연결 설정](#aws-ecr-repository-보안-취약점-스캔-연결-설정)
  - [AWS ECR 명령어](#aws-ecr-명령어)
  - [실습 파일](#실습-파일-3)
- [24. docker 트러블 슈팅](#24-docker-트러블-슈팅)
  - [Docker Network 트래픽 Dump](#docker-network-트래픽-dump)
  - [Docker Network 트래픽 Debugging](#docker-network-트래픽-debugging)
  - [Private 환경에서 Nexus 활용](#private-환경에서-nexus-활용)
  - [Private 환경에서 Squid Proxy 활용](#private-환경에서-squid-proxy-활용)
  - [Docker 데몬 상태 확인 및 이슈 로깅 명령어](#docker-데몬-상태-확인-및-이슈-로깅-명령어)
  - [CRI-O 데몬 상태 확인 및 이슈 로깅 명령어](#cri-o-데몬-상태-확인-및-이슈-로깅-명령어)
  - [tcpdump 명령어](#tcpdump-명령어)
  - [wireshark에 tcpdump 파일 import 및 디버깅 수행](#wireshark에-tcpdump-파일-import-및-디버깅-수행)
